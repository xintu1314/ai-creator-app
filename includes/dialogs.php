<?php
// 模型选择对话框（图片/视频模型分别渲染，由 JS 切换显示）
$defaultAspectRatio = $defaultAspectRatio ?? (($creationType ?? 'image') === 'video' ? '16:9' : '3:4');
$currentModel = $currentModel ?? (($creationType ?? 'image') === 'video' ? ($videoModels[0] ?? ['id' => '']) : ($imageModels[0] ?? ['id' => '']));
?>
<!-- Model Select Dialog -->
<div id="model-dialog" class="hidden fixed inset-0 z-50 dialog-overlay" onclick="closeModelDialog()" style="display: none;">
    <div class="dialog-content max-w-[680px] p-0 gap-0 overflow-hidden" onclick="event.stopPropagation()">
        ...
        <div class="p-4 max-h-[500px] overflow-y-auto">
            <!-- 图片模型列表 -->
            <div id="model-dialog-image-list" class="grid grid-cols-2 gap-3 model-dialog-list">
                <?php foreach ($imageModels as $model): ?>
                    <?php include __DIR__ . '/_model_option.php'; ?>
                <?php endforeach; ?>
            </div>
            <!-- 视频模型列表 -->
            <div id="model-dialog-video-list" class="grid grid-cols-2 gap-3 model-dialog-list hidden">
                <?php foreach ($videoModels as $model): ?>
                    <?php include __DIR__ . '/_model_option.php'; ?>
                <?php endforeach; ?>
            </div>
        </div>
...
<!-- Auth Dialog：登录与注册分开，登录分验证码/密码 -->
<div id="auth-dialog" class="hidden fixed inset-0 z-50 dialog-overlay" style="display: none;" onclick="if(event.target===this)closeAuthDialog()">
    <div class="dialog-content max-w-[420px] w-[92vw] p-0 gap-0 overflow-hidden rounded-2xl shadow-xl" onclick="event.stopPropagation()" style="pointer-events: auto;">
        <!-- 第一步：选择登录或注册 -->
        <div id="auth-step-choose" class="p-5">
            <div class="relative px-2 pb-4 overflow-hidden">
                <div class="absolute inset-0 opacity-30 pointer-events-none" style="background-image: radial-gradient(circle, #fff 1px, transparent 1px); background-size: 12px 12px;"></div>
                <h3 class="relative text-[22px] font-bold text-[#0C4A6E] mb-1">欢迎使用</h3>
                <p class="relative text-sm text-[#64748B]">请选择登录或注册</p>
            </div>
            <div class="space-y-3 mt-4">
                <button type="button" onclick="authShowLogin()" class="w-full h-12 text-base font-semibold bg-[#2563EB] hover:bg-[#1D4ED8] text-white rounded-xl cursor-pointer transition-colors flex items-center justify-center">
                    登录
                </button>
                <button type="button" onclick="authShowRegister()" class="w-full h-12 text-base font-semibold bg-white hover:bg-[#F8FAFC] text-[#2563EB] border-2 border-[#2563EB] rounded-xl cursor-pointer transition-colors flex items-center justify-center">
                    注册
                </button>
            </div>
        </div>
        ...
        <div id="auth-step-login" class="hidden">
            <div class="relative px-5 pt-5 pb-2 overflow-hidden">
                <div class="absolute inset-0 opacity-30 pointer-events-none" style="background-image: radial-gradient(circle, #fff 1px, transparent 1px); background-size: 12px 12px;"></div>
                <button type="button" onclick="authBackToChoose()" class="relative text-sm text-[#64748B] hover:text-[#2563EB] cursor-pointer mb-2">← 返回</button>
                <h3 class="relative text-[20px] font-bold text-[#0C4A6E]">登录</h3>
                <p id="auth-login-subtitle" class="relative text-xs text-[#0369A1]/90 mt-1">验证码登录（需已注册）</p>
            </div>
            <div class="flex mx-5 mt-2 mb-2 bg-[#F1F5F9] rounded-xl p-1">
                <button type="button" id="auth-tab-code" onclick="switchLoginTab('code')" class="flex-1 h-9 text-sm font-medium rounded-lg transition-colors bg-white text-[#2563EB] shadow-sm cursor-pointer">验证码登录</button>
                <button type="button" id="auth-tab-password" onclick="switchLoginTab('password')" class="flex-1 h-9 text-sm font-medium rounded-lg transition-colors text-[#64748B] cursor-pointer">密码登录</button>
            </div>
            <form id="auth-login-form" class="p-5 space-y-4 bg-white" onsubmit="submitLoginForm(event)">
                <input type="hidden" id="auth-login-mode" value="code">
                <div>
                    <label class="text-sm text-[#64748B] mb-2 block font-medium">手机号</label>
                    <div class="flex items-center h-12 px-4 rounded-xl bg-[#F8FAFC] border border-[#E2E8F0] focus-within:border-[#3B82F6] transition-all">
                        <span class="text-[#334155] text-sm font-medium">+86</span>
                        <div class="w-px h-5 mx-3 bg-[#CBD5E1]"></div>
                        <input id="auth-login-phone" type="text" maxlength="11" autocomplete="tel" placeholder="请输入手机号" class="flex-1 h-full text-sm bg-transparent outline-none border-0" />
                    </div>
                </div>
                <div id="auth-login-code-wrap">
                    <label class="text-sm text-[#64748B] mb-2 block font-medium">短信验证码</label>
                    <div class="flex items-center gap-2">
                        <input id="auth-login-code" type="text" maxlength="6" inputmode="numeric" placeholder="请输入6位验证码" class="flex-1 h-12 px-4 text-sm rounded-xl bg-[#F8FAFC] border border-[#E2E8F0] focus:border-[#3B82F6] outline-none" />
                        <button id="auth-send-code-btn" type="button" onclick="sendLoginCode()" class="h-12 px-4 text-sm font-medium text-[#1D4ED8] bg-[#EFF6FF] border border-[#BFDBFE] rounded-xl hover:bg-[#DBEAFE] cursor-pointer whitespace-nowrap">
                            获取验证码
                        </button>
                    </div>
                </div>
                <div id="auth-login-password-wrap" class="hidden">
                    <label class="text-sm text-[#64748B] mb-2 block font-medium">密码</label>
                    <input id="auth-login-password" type="password" autocomplete="current-password" placeholder="请输入密码（6-64位）" class="w-full h-12 px-4 text-sm rounded-xl bg-[#F8FAFC] border border-[#E2E8F0] focus:border-[#3B82F6] outline-none" />
                    <p class="mt-1.5 text-right"><button type="button" onclick="authShowReset()" class="text-xs text-[#3B82F6] hover:underline cursor-pointer">忘记密码？</button></p>
                </div>
                <div id="auth-login-error" class="hidden text-xs text-red-600 bg-red-50 border border-red-100 rounded-xl px-3 py-2.5"></div>
                <button type="submit" id="auth-login-submit" class="w-full h-12 text-base font-semibold bg-[#2563EB] hover:bg-[#1D4ED8] text-white rounded-xl cursor-pointer transition-colors">
                    登录
                </button>
                <p class="text-center text-sm text-[#94A3B8]">还没有账号？<button type="button" onclick="authShowRegister()" class="text-[#3B82F6] hover:underline cursor-pointer">去注册</button></p>
            </form>
        </div>

        <!-- 忘记密码：重置密码表单 -->
        <div id="auth-step-reset" class="hidden">
            <div class="relative px-5 pt-5 pb-2 overflow-hidden">
                <div class="absolute inset-0 opacity-30 pointer-events-none" style="background-image: radial-gradient(circle, #fff 1px, transparent 1px); background-size: 12px 12px;"></div>
                <button type="button" onclick="authBackToLogin()" class="relative text-sm text-[#64748B] hover:text-[#2563EB] cursor-pointer mb-2">← 返回登录</button>
                <h3 class="relative text-[20px] font-bold text-[#0C4A6E]">忘记密码</h3>
                <p class="relative text-xs text-[#0369A1]/90 mt-1">请输入手机号、验证码和新密码</p>
            </div>
            <form id="auth-reset-form" class="p-5 space-y-4 bg-white" onsubmit="submitResetForm(event)">
                <div>
                    <label class="text-sm text-[#64748B] mb-2 block font-medium">手机号</label>
                    <div class="flex items-center h-12 px-4 rounded-xl bg-[#F8FAFC] border border-[#E2E8F0] focus-within:border-[#3B82F6] transition-all">
                        <span class="text-[#334155] text-sm font-medium">+86</span>
                        <div class="w-px h-5 mx-3 bg-[#CBD5E1]"></div>
                        <input id="auth-reset-phone" type="text" maxlength="11" autocomplete="tel" placeholder="请输入手机号" class="flex-1 h-full text-sm bg-transparent outline-none border-0" />
                    </div>
                </div>
                <div>
                    <label class="text-sm text-[#64748B] mb-2 block font-medium">短信验证码</label>
                    <div class="flex items-center gap-2">
                        <input id="auth-reset-code" type="text" maxlength="6" inputmode="numeric" placeholder="请输入6位验证码" class="flex-1 h-12 px-4 text-sm rounded-xl bg-[#F8FAFC] border border-[#E2E8F0] focus:border-[#3B82F6] outline-none" />
                        <button id="auth-reset-send-btn" type="button" onclick="sendResetCode()" class="h-12 px-4 text-sm font-medium text-[#1D4ED8] bg-[#EFF6FF] border border-[#BFDBFE] rounded-xl hover:bg-[#DBEAFE] cursor-pointer whitespace-nowrap">
                            获取验证码
                        </button>
                    </div>
                </div>
                <div>
                    <label class="text-sm text-[#64748B] mb-2 block font-medium">新密码</label>
                    <input id="auth-reset-password" type="password" autocomplete="new-password" placeholder="6-64位新密码" class="w-full h-12 px-4 text-sm rounded-xl bg-[#F8FAFC] border border-[#E2E8F0] focus:border-[#3B82F6] outline-none" />
                </div>
                <div id="auth-reset-error" class="hidden text-xs text-red-600 bg-red-50 border border-red-100 rounded-xl px-3 py-2.5"></div>
                <button type="submit" id="auth-reset-submit" class="w-full h-12 text-base font-semibold bg-[#2563EB] hover:bg-[#1D4ED8] text-white rounded-xl cursor-pointer transition-colors">
                    重置密码
                </button>
                <p class="text-center text-sm text-[#94A3B8]">想起密码？<button type="button" onclick="authBackToLogin()" class="text-[#3B82F6] hover:underline cursor-pointer">返回登录</button></p>
            </form>
        </div>

        <!-- 第二步：注册表单 -->
        <div id="auth-step-register" class="hidden">
            <div class="relative px-5 pt-5 pb-2 overflow-hidden">
                <div class="absolute inset-0 opacity-30 pointer-events-none" style="background-image: radial-gradient(circle, #fff 1px, transparent 1px); background-size: 12px 12px;"></div>
                <button type="button" onclick="authBackToChoose()" class="relative text-sm text-[#64748B] hover:text-[#2563EB] cursor-pointer mb-2">← 返回</button>
                <h3 class="relative text-[20px] font-bold text-[#0C4A6E]">注册</h3>
                <p class="relative text-xs text-[#0369A1]/90 mt-1">请输入手机号、验证码和密码完成注册</p>
            </div>
            <form id="auth-register-form" class="p-5 space-y-4 bg-white" onsubmit="submitRegisterForm(event)">
                <div>
                    <label class="text-sm text-[#64748B] mb-2 block font-medium">手机号</label>
                    <div class="flex items-center h-12 px-4 rounded-xl bg-[#F8FAFC] border border-[#E2E8F0] focus-within:border-[#3B82F6] transition-all">
                        <span class="text-[#334155] text-sm font-medium">+86</span>
                        <div class="w-px h-5 mx-3 bg-[#CBD5E1]"></div>
                        <input id="auth-register-phone" type="text" maxlength="11" autocomplete="tel" placeholder="请输入手机号" class="flex-1 h-full text-sm bg-transparent outline-none border-0" />
                    </div>
                </div>
                <div>
                    <label class="text-sm text-[#64748B] mb-2 block font-medium">短信验证码</label>
                    <div class="flex items-center gap-2">
                        <input id="auth-register-code" type="text" maxlength="6" inputmode="numeric" placeholder="请输入6位验证码" class="flex-1 h-12 px-4 text-sm rounded-xl bg-[#F8FAFC] border border-[#E2E8F0] focus:border-[#3B82F6] outline-none" />
                        <button id="auth-register-send-btn" type="button" onclick="sendRegisterCode()" class="h-12 px-4 text-sm font-medium text-[#1D4ED8] bg-[#EFF6FF] border border-[#BFDBFE] rounded-xl hover:bg-[#DBEAFE] cursor-pointer whitespace-nowrap">
                            获取验证码
                        </button>
                    </div>
                </div>
                <div>
                    <label class="text-sm text-[#64748B] mb-2 block font-medium">密码</label>
                    <input id="auth-register-password" type="password" autocomplete="new-password" placeholder="6-64位密码" class="w-full h-12 px-4 text-sm rounded-xl bg-[#F8FAFC] border border-[#E2E8F0] focus:border-[#3B82F6] outline-none" />
                </div>
                <div>
                    <label class="text-sm text-[#64748B] mb-2 block font-medium">昵称（选填）</label>
                    <input id="auth-register-nickname" type="text" maxlength="100" placeholder="不填则使用手机号昵称" class="w-full h-12 px-4 text-sm rounded-xl bg-[#F8FAFC] border border-[#E2E8F0] focus:border-[#3B82F6] outline-none" />
                </div>
                <div id="auth-register-error" class="hidden text-xs text-red-600 bg-red-50 border border-red-100 rounded-xl px-3 py-2.5"></div>
                <button type="submit" id="auth-register-submit" class="w-full h-12 text-base font-semibold bg-[#2563EB] hover:bg-[#1D4ED8] text-white rounded-xl cursor-pointer transition-colors">
                    注册
                </button>
                <p class="text-center text-sm text-[#94A3B8]">已有账号？<button type="button" onclick="authShowLogin()" class="text-[#3B82F6] hover:underline cursor-pointer">去登录</button></p>
            </form>
        </div>

        <p class="text-[11px] text-center text-[#94A3B8] pb-4">
